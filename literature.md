...
2. Program Slicing in the Era of Large Language Models
Overview: This study evaluates how four state-of-the-art LLMs (GPT-4o, GPT-3.5 Turbo, Llama-2, and Gemma-7B) perform on static and dynamic program slicing tasks in Java. The authors find that while GPT-4o performs best, current LLMs still struggle significantly with slicing and have not yet reached a reasonable level of performance.

Key Definitions and Conceptual Framing: Static slicing analyzes code without execution to map dependencies, while dynamic slicing captures runtime behavior based on concrete execution traces. The paper frames its analysis around prompt engineering techniques: zero-shot, one-shot, and Chain-of-Thought (CoT) reasoning.

Historical Trajectory: Historically, developers relied on traditional program slicing tools (like JavaSlicer and Slicer4J), which are reliable but heavily dependent on strict algorithms. The advent of LLMs introduced NLP-driven program analysis, but this paper marks an inflection point by testing if LLMs can inherently grasp strict deterministic logic like control flow.

Scope and Applications: The evaluation is conducted on a dataset of 100 preprocessed Java programs derived from LeetCode. The slices were evaluated against ground truth generated by traditional tools (JavaSlicer and Slicer4J) to aid in bug detection and code comprehension.

Commentary on Modern Directions: Manual analysis revealed a taxonomy of LLM failures, pinpointing "Complex Control Flow" as the main root cause and "Variable Declarations and Assignments" as the primary failure location. The authors suggest that Iterative Prompting and Enhanced Prompt Crafting directly addressing these weaknesses can yield slight accuracy improvements.

Significance for the Literature Review: It critically exposes the limitations of LLMs regarding complex semantic reasoning and deterministic path-tracing. It provides a novel failure taxonomy that can guide future researchers in structuring prompts or hybridizing LLMs with traditional static analysis tools.

3. On the Role of Context Granularity in LLM-Driven Program Repair
...
4. Program Decomposition and Translation with Static Analysis
...
5. Slicer4J: A Dynamic Slicer for Java
Overview: The paper introduces Slicer4J, a dynamic slicing tool designed for modern Java. It relies on low-overhead execution trace instrumentation and builds a thread-aware dynamic control-flow graph to accurately track dependencies and compute slices.

Key Definitions and Conceptual Framing: A dynamic execution trace is the sequence of executed statements from a specific run. The slicing criterion consists of a specific executed statement and the variables it uses. A Thread-aware, Inter-procedural Dynamic Control-flow Graph (TDCG) is the core data structure used to untangle multithreaded runtime dependencies.

Historical Trajectory: The only publicly available dynamic slicer for Java before this was JavaSlicer. However, JavaSlicer became obsolete because it was hard-capped at Java 6, failed to support multithreading (producing fragmented single-thread slices), and could not handle native framework methods. Slicer4J was built to overcome these modern language limitations.

Scope and Applications: Slicer4J is evaluated on 10 benchmark programs and massive open-source programs from the Defects4J dataset. It is designed for fault localization, debugging, regression testing, and security/malware analysis.

Commentary on Modern Directions: Slicer4J handles native and framework methods not by instrumenting them at runtime (which causes infinite loops), but by using a pre-constructed list of static taint-flow summaries. A noted limitation for future work is handling dynamically generated runtime classes.

Significance for the Literature Review: This tool functions as the technical foundation that enables the static and dynamic analysis pipelines discussed in other papers in this set (e.g., Paper 2 explicitly uses Slicer4J to establish ground truths). It is critical for generating the dependency graphs required by modern AI-augmented debugging frameworks.
